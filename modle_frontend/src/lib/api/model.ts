...
export async function getModels(
  ...
  const safeQuery: Record<string, string | string[] | undefined> = {
    ...params,
  };
  ...
  if (
    ...
    return {
      ...
      totalPages:
        pageData.totalPages ||
        Math.ceil((pageData.totalElements || 0) / 12) ||
        1,
...
export async function getMyModel(customClient?: typeof client): Promise<Model> {
  const c = customClient ?? client;
  const { data, error } = await c.GET("/api/v1/models/my");

  if (error) {
    throw new Error(
      (error as { msg?: string })?.msg ||
        "내 모델 정보를 불러오는데 실패했습니다.",
    );
  }
